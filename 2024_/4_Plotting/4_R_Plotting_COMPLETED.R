...
dim(milk)
milk <- merge(milk,plant_type,by='plant')
dim(milk)
...
# basic scatter plot
plot(x=1:10,y=seq(from=5,to=50,by=5))
plot(x=1:10,y=seq(from=5,to=60,by=5)) #notice that x and y vectors need to be of same length to be plotted

# change scatter point shape (http://www.sthda.com/english/wiki/r-plot-pch-symbols-the-different-point-shapes-available-in-r)
plot(x=1:10,y=seq(from=5,to=50,by=5),pch=6)
plot(x=1:10,y=seq(from=5,to=50,by=5),pch="*")

# line graph
setorder(game_data,Year)
plot(x=game_data[Name=='Monopoly',Year],y=game_data[Name=='Monopoly',Global_Sales],type='l')
game_data[Name=='Monopoly',plot(x=Year,y=Global_Sales,type='l')]

# change line width
game_data[Name=='Monopoly',plot(x=Year,y=Global_Sales,type='l',lwd=3)]

# change line type
game_data[Name=='Monopoly',plot(x=Year,y=Global_Sales,type='l',lwd=3,lty='dashed')]

# change axis titles
game_data[Name=='Monopoly',plot(x=Year,y=Global_Sales,type='l',lwd=3,xlab='Year',ylab='Global sales (million $)')]

# specify title and caption
game_data[Name=='Monopoly',plot(x=Year,y=Global_Sales,
                                type='l',
                                lwd=3,
                                xlab='Year',
                                ylab='Global sales (million $)',
                                main='Monopoly global sales over time',
                                sub='Data taken from global game data database 2022')]

# basic histogram of global sales
hist(game_data$Global_Sales)
game_data[,hist(Global_Sales)]
game_data[Global_Sales<5,hist(Global_Sales)]

# change the number of breaks/bins
game_data[Global_Sales<5,hist(Global_Sales,breaks=10)]
...
# baisc scatter plot
ggplot(data=milk) +
  geom_point(mapping=aes(x=kcal,y=protein))
ggplot() +
  geom_point(data=milk,mapping=aes(x=kcal,y=protein))
